        ...
        <section className="manager-page stack">
          ...
          <div className="content-grid content-grid-wide">
            <article className="panel stack">
              <div className="panel-heading"><div><p className="eyebrow">Reserve focus</p><h2>Your busiest reserves</h2></div><button type="button" className="secondary-button" onClick={() => setPage('requests')}>Open requests page</button></div>
              <div className="overview-reserve-grid">{reserveSummaries.length === 0 ? <p className="empty-state">No active reserves yet.</p> : reserveSummaries.slice().sort((left, right) => (right.priorityCounts.HIGH - left.priorityCounts.HIGH) || (right.totalActiveEvents - left.totalActiveEvents)).slice(0, 3).map((reserve) => <article key={reserve.id} className="overview-reserve-card"><div className="overview-reserve-header"><div><strong>{reserve.displayName}</strong><span>{reserve.region}</span></div><button type="button" className="secondary-button" onClick={() => openReserveControlCenter(reserve.id)}>Open control center</button></div><div className="event-meta"><span>{reserve.totalActiveEvents} active events</span><span>{reserve.travelerReports} traveler reports</span></div><PriorityBreakdown counts={reserve.priorityCounts} /></article>)}</div>
            </article>
            <article className="panel stack">
        ...
        <section className="manager-page stack">
          {selectedReserve ? <>
            ...
            <div className="control-center-layout">
              <article className="panel control-map-panel">
                <div className="panel-heading"><div><p className="eyebrow">Reserve map</p><h2>{selectedReserve.displayName} area</h2></div><PriorityBreakdown counts={selectedReserve.priorityCounts} /></div>
              ...
              <aside className="panel control-sidebar stack">
                <section>
                  <div className="panel-heading"><div><p className="eyebrow">Filters</p><h2>Visible events</h2></div></div>
          ...
          <div className="content-grid content-grid-wide">
            <article className="panel stack">
              ...
              <form className="event-form" onSubmit={submitReserveRequest}>
                ...
                <button type="submit">Send reserve request</button>
              </form>
            </article>
            <article className="panel stack">
